...
The report will automatically open in the UI so the user can watch it being built.
IMPORTANT: After creating a report, you MUST immediately call execute_typescript to add charts, metrics, and other components. Do NOT stop after creating the report.`,
  ...
  return {
    success: true,
    reportId: id,
    message: `Report "${title}" created. NOW call execute_typescript immediately with code that uses external_queryTable to fetch data, then external_report_card, external_report_chart, external_report_metric etc. to build the report UI. Do NOT just return text.`,
  }
})
